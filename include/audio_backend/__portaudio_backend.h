...
// multi-API problem : can't e.g. intiialize jack from itself, and also from portaudio
//#include <audio>
...
{
public:
  explicit __dynamic_library(const char* const so) noexcept
  {
#ifdef _WIN32
...
// How to handle stuff like MME or pulseaudio where you can mix and match sound cards ?
template<>
class audio_basic_device<__portaudio_driver_t>
{
...
public:
      ...
      , m_index{device}
    {
        ...
        m_frames = std::max(m_info.defaultHighInputLatency, m_info.defaultHighOutputLatency) * m_sampleRate;
    }

    audio_basic_device(const audio_basic_device& other) : audio_basic_device(other.m_info, other.m_index)
    {

    }
    audio_basic_device(audio_basic_device&& other) : audio_basic_device(other.m_info, other.m_index)
    {
    ...
    ~audio_basic_device()
    {
        __portaudio_lib::instance().terminate();
    }
    ...
    bool start() {
        ...
        // actual buffer size until it's running - and even then it's only an estimation
        // - on some platforms it can vary per-tick
        auto& pa = __portaudio_lib::instance();
        auto err = pa.open_stream(&m_stream, actualInput, actualOutput, m_sampleRate, m_frames, paNoFlag, callback, this);
        if(err != paNoError) {
    ...
    bool is_input() const noexcept {
        return m_in.channelCount > 0;
    }

    bool is_output() const noexcept {
        return m_out.channelCount > 0;
    }

    int get_num_input_channels() const noexcept {
        return m_in.channelCount;
    }

    int get_num_output_channels() const noexcept {
        return m_out.channelCount;
    }

    template<typename T>
    void connect(T t) {
        // in my experience, it is sometimes necessary to swap callbacks at run-time.
        // that's doable with some atomic operations.
        if (is_running())
    ...
    // Problem for this : depending on the API, it's a pair <sample rate, buffer size> which is supported.
    ...
    span<const buffer_size_t> get_supported_buffer_sizes_frames() const noexcept {
        static constexpr std::array<buffer_size_t, 7> bs{ paFramesPerBufferUnspecified, 32, 64, 128, 256, 512, 1024 };
...
private:
    ...
    using __portaudio_callback_t = function<void(__portaudio_device&, audio_device_buffers&)>;
            ...
            PaStreamCallbackFlags statusFlags, void* userData)
    {
        ...
        if(self.m_callback)
        {
            ...
            // Right now most efficient way I know of doing conversions is using FFMPEG's libsamplerate.
            // How would that fit in ?
            using buffer_t = audio_basic_buffer<float, std::experimental::audio_buffer_order_interleaved>;
            if(input)
            {
                // needs a const_cast here :x
                buffer_t inb((float*) input, nframes * self.get_num_input_channels(), self.get_num_input_channels());
                bufs.__input_buffer = inb;
            }

            if(output)
            {
                buffer_t outb((float*) output, nframes * self.get_num_output_channels(), self.get_num_output_channels());
...
template<>
class audio_basic_device_list<__portaudio_driver_t> : public forward_list<__portaudio_device>
{
    const __portaudio_lib& library;
    __portaudio_lib::scoped_init_context context;
public:
    audio_basic_device_list(bool init_inputs, bool init_outputs)
        : library{__portaudio_lib::instance()}
        , context{library}
    {
        for (PaDeviceIndex i = 0, n = library.get_device_count(); i < n; i++)
        {
            try {
                ...
                if(!dev)
                    continue;
                if((init_inputs && dev->maxInputChannels > 0) || (init_outputs && dev->maxOutputChannels > 0))
